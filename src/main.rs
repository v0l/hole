...
use nostr_relay_builder::builder::{PolicyResult, QueryPolicy, RateLimit, WritePolicy};
use nostr_relay_builder::prelude::{
    async_trait, Backend, Coordinate, DatabaseError, DatabaseEventStatus, Event, EventId, Events,
    JsonUtil, Kind, NostrDatabase, RejectedReason, Timestamp,
};
use nostr_relay_builder::prelude::{
...
use nostr_sdk::{Client, FilterOptions};
...
#[derive(Debug)]
struct KindPolicy(HashSet<Kind>);
...
async fn main() -> Result<()> {
    ...
    if let Some(r) = config.relays {
        ...
        let mut stream = client
            .pool()
            .stream_events(
                vec![nostr_sdk::Filter::new().limit(100)],
                never,
                FilterOptions::WaitDurationAfterEOSE(never),
            )
            .await?;
        let db = db.clone();
        tokio::spawn(async move {
    ...
    let mut b = RelayBuilder::default()
        .database(db.clone())
        .addr(addr.ip())
        .port(addr.port())
